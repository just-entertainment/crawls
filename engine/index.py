...
def single_product(href, category_name):
    try:
        ...
        save_dir = Path(f"./{category_name}/{cleaned_product_name}")
        ...
        if tables:
            ...
            for tr in trs:
                # 信息有三项 id model Description 有的Description含有可以下载的pdf 有的没有
                columns = tr.children()
                ...
                if 'Free download.' in description:
                    try:
                        ...
                        if pdf_href:
                            url = pdf_href
                            save_path = f'./{category_name}/{cleaned_product_name}/'
                            # 使用新的断点续传下载函数
            ...
            # 一次性保存所有数据到CSV文件
            if csv_data:
                csv_filepath = f'./{category_name}/{cleaned_product_name}/{cleaned_product_name}.csv'
                save_to_csv(csv_data, csv_filepath)
            
            print(f"已保存 {len(csv_data)} 条记录到 {cleaned_product_name}.csv")

        else:
            # 如果没有表格，下载页面内所有PDF文件
            pdf_links = page1.eles('tag:a:has-text(.pdf)')  # 查找包含.pdf的链接
            if not pdf_links:
                # 如果没有找到带.pdf文本的链接，查找所有可能的PDF链接
                all_links = page1.eles('tag:a')
                pdf_links = [link for link in all_links if link.attr('href') and '.pdf' in link.attr('href').lower()]
            ...
            for pdf_link in pdf_links:
                ...
                if pdf_url:
                    ...
                    try:
                        # 使用新的断点续传下载函数
                        res = resume_download(pdf_url, save_path)  # 下载pdf
                        print(f"下载PDF: {res}")
                    except Exception as pdf_error:
                        print(f"下载PDF时出错: {pdf_error}")
